...
public class ImportationVisite extends AsyncTask<String,Void,ArrayList<Visite>> {
    @Override
    protected ArrayList<Visite> doInBackground(String... params) {
    try {

        URL url = new URL(params[0]);
        ArrayList<Visite> listeVisite;
        ...
        if(httpResult== HttpURLConnection.HTTP_OK){
           ...
           // System.out.println(ligne);
            Gson gson=new Gson();
            listeVisite = gson.fromJson(ligne,new TypeToken<ArrayList<Visite>>(){}.getType());
            return listeVisite;
